...
(async () => {
  const browser = await puppeteer.launch({
    headless: true,  // Headed mode for debugging
    devtools: true,   // Open DevTools for further debugging if needed
  });
...
(async () => {
  ...
  const result = await page.evaluate(() => {
    return new Promise((resolve) => {
      ...
      QUnit.done((details) => {
        resolve({ end: end, details: details, failures: failures, logs: logs });
      });
  ...
  //console.log('total ends: ', result.end.length);

  //console.dir(result.end, { depth: null, colors: true });

  //console.dir(result.logs, { depth: null, colors: true });
